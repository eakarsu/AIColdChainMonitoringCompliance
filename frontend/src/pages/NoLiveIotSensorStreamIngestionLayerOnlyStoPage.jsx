...
export default function NoLiveIotSensorStreamIngestionLayerOnlyStoPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-no-live-iot-sensor-stream-ingestion-layer-only-sto/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>No live IoT-sensor stream ingestion layer (only stored temperature logs)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-no-live-iot-sensor-stream-ingestion-layer-only-sto/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
