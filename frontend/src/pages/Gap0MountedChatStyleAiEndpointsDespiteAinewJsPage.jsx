...
export default function Gap0MountedChatStyleAiEndpointsDespiteAinewJsPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-0-mounted-chat-style-ai-endpoints-despite-ainew-js/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>0 mounted /chat-style AI endpoints despite aiNew.js scaffold</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-0-mounted-chat-style-ai-endpoints-despite-ainew-js/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
