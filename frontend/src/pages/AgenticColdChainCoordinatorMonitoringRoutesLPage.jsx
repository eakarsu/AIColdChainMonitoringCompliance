...
export default function AgenticColdChainCoordinatorMonitoringRoutesLPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-cold-chain-coordinator-monitoring-routes-l/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic cold-chain coordinator monitoring routes, lots, and excursions with automatic remediation</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-cold-chain-coordinator-monitoring-routes-l/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
