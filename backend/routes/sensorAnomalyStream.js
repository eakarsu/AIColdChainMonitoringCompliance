/*
 * Sensor anomaly stream: Anomaly detection on sensor streams with playbooks
 *
 * Custom Feature Suggestion (apply pass 6 — audit batch_01)
 * Suggestion: Anomaly detection on sensor streams with playbooks
 *
...
router.post('/run', async (req, res) => {
  try {
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({ error: 'AI unavailable', missing: 'OPENROUTER_API_KEY', feature: 'sensor_anomaly' });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${'Sensor anomaly stream'}\n\nReturn JSON {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const systemPrompt = `You are a $'cold-chain sensor anomaly detector'. Respond with strict JSON.`;
    const aiResp = await callOpenRouter(userPrompt, systemPrompt);
    const content = aiResp?.choices?.[0]?.message?.content ?? aiResp?.raw ?? '';
    const parsed = parseAIJson(content);
    return res.json({ feature: 'sensor_anomaly', model: aiResp?.model, parsed, raw: content });
  } catch (err) {
...
router.get('/health', (req, res) => res.json({ feature: 'sensor_anomaly', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
